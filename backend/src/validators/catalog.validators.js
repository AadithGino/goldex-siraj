...
export const storeSettingsBody = z.object({
  ...
  currency_symbol: z.string().trim().max(10).optional(),
  country_code: z.string().trim().min(2).max(2).optional(),
  support_email: z.union([z.string().trim().email().max(200), z.literal(''), z.null()]).optional(),
  ...
  address: z.union([
    ...
    z.object({
      ...
      emirate: z.string().trim().max(120).optional(),
  ...
  branches: z.array(z.object({
    ...
    maps_url: z.string().trim().max(2048).optional(),
    is_primary: z.boolean().optional(),
  }).strip()).max(20).optional(),
  social_links: z.record(z.string().trim().max(500)).optional(),
  gold_scheme_enabled: z.boolean().optional(),
  cod_enabled: z.boolean().optional(),
  online_payment_enabled: z.boolean().optional(),
  bank_transfer_enabled: z.boolean().optional(),
  cod_min_order_amount: z.coerce.number().finite().min(0).optional(),
  cod_min_order: z.coerce.number().finite().min(0).optional(),
  cod_max_order_amount: z.union([z.coerce.number().finite().min(0), z.null()]).optional(),
  minimum_order_amount: z.coerce.number().finite().min(0).optional(),
  shipping_fee: z.coerce.number().finite().min(0).optional(),
  free_shipping_threshold: z.coerce.number().finite().min(0).optional(),
  return_window_days: z.coerce.number().int().min(0).max(365).optional(),
