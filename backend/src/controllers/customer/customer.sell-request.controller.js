...
export async function reply(req, res) {
  const id = req.validated?.params?.id ?? req.params.id
  const body = req.validated?.body || req.body || {}
  await ok(res, serialize(await service.replyToRequest(req.auth.sub, id, body)))
}

export async function accept(req, res) {
  const id = req.validated?.params?.id ?? req.params.id
  await ok(res, serialize(await service.acceptOffer(req.auth.sub, id)))
}

export async function decline(req, res) {
  const id = req.validated?.params?.id ?? req.params.id
  const body = req.validated?.body || req.body || {}
  await ok(res, serialize(await service.declineOffer(req.auth.sub, id, body.reason)))
}
