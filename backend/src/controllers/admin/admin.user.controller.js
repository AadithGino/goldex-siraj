...
export async function customer(req, res) {
  ok(res, serialize(await service.getCustomer(req.validated?.params?.id ?? req.params.id)))
}
export async function customerUpdate(req, res) {
  ok(res, serialize(await service.updateCustomer(req.validated?.params?.id ?? req.params.id, req.validated.body)))
}
export async function staff(_req, res) {
  ok(res, serialize(await service.listStaff()))
}
export async function staffCreate(req, res) {
  ok(res, serialize(await service.createStaff(req.validated.body, req.auth.sub)), 201)
}
export async function staffUpdate(req, res) {
  ok(res, serialize(await service.updateStaff(req.validated?.params?.id ?? req.params.id, req.validated.body, req.auth.sub)))
}
...
export async function customerWallet(req, res) {
  ...
  ok(res, serialize(await walletService.transactions(req.validated?.params?.id ?? req.params.id, limit)))
}
