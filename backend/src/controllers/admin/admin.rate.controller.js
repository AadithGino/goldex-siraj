...
export async function goldBuybackList(req, res) { await ok(res, serialize(await rateService.listGoldBuybackRates(req.query.current === 'true'))) }
export async function goldBuybackSet(req, res) { await ok(res, serialize(await rateService.setGoldBuybackRate(req.body, req.auth.sub)), 201) }
