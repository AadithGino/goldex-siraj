...
export async function create(req, res) {
  await ok(res, serialize(await service.createScheme(req.validated.body)), 201)
}

export async function update(req, res) {
  await ok(res, serialize(await service.updateScheme(req.validated.params.id, req.validated.body)))
}

export async function enrollments(req, res) {
  const result = await service.listEnrollments(req.validated?.query ?? req.query)
  await ok(res, result.items, 200, {
    page: result.page, limit: result.limit, total: result.total, pages: result.pages,
  })
}
...
export async function completeEnrollment(req, res) {
  ...
  await ok(res, serialize(await service.completeEnrollment(
    req.validated.params.id,
    req.auth.sub,
...
export async function recordPayment(req, res) {
  await ok(res, await service.recordInstallment(
    req.validated.params.id,
...
export async function recordPaymentById(req, res) {
  await ok(res, await service.recordInstallmentById(
    req.validated.params.installmentId,
    req.validated.body,
    req.auth.sub,
  ))
