...
export async function couponCreate(req, res) {
  ok(res, serialize(await service.createCoupon(req.validated.body)), 201)
}
export async function couponUpdate(req, res) {
  ok(res, serialize(await service.updateCoupon(req.validated?.params?.id ?? req.params.id, req.validated.body)))
}
export async function couponDelete(req, res) {
  ok(res, serialize(await service.deleteCoupon(req.validated?.params?.id ?? req.params.id)))
}
export async function couponUsageSummary(_req, res) {
  ok(res, serialize(await service.couponUsageSummary()))
}
...
export async function adjust(req, res) {
  ...
  ok(res, serialize(await service.adjustStock(
    req.validated?.params?.id ?? req.params.id,
    body.delta,
    body.reason,
    body.note,
...
export async function setStock(req, res) {
  const body = req.validated.body
  ok(res, serialize(await service.setStock(req.validated?.params?.id ?? req.params.id, {
    qty: body.qty,
